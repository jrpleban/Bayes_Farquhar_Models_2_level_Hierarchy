...
source("CaCc_Temp_Jf_model_H.R") ### Bayesian Modelw/ Temp Dependency & farQ_tempDep limitation

#### Set up for rjags #########
parameters = c("Vcmax25", "Rd25","gammaS25","Kc25", "Ko25",
"mu.Vcmax25", "mu.Rd25","mu.gammaS25",
"tau.Vcmax25", "tau.Rd25","tau.gammaS25",
"EgammaS" ,
"ERd" ,"EKc", "EKo",  "EVcmax","tau")
...
datalist1<-list(N=length(A[[1]]), NID=length(unique(IDdat[[1]])), An=A[[1]], Ca=CP[[1]],T=T[[1]], K=K[[1]], g=g[[1]],  Jf=Jf[[1]], O=O[[1]], ID=IDdat[[1]], Kref=298.15, R=0.008314, Tref=25) #301
datalist2<-list(N=length(A[[2]]), NID=length(unique(IDdat[[2]])), An=A[[2]], Ca=CP[[2]], T=T[[2]],K=K[[2]], g=g[[2]],  Jf=Jf[[2]], O=O[[2]], ID=IDdat[[2]], Kref=298.15, R=0.008314, Tref=25) #46
datalist3<-list(N=length(A[[3]]), NID=length(unique(IDdat[[3]])), An=A[[3]], Ca=CP[[3]], T=T[[3]],K=K[[3]], g=g[[3]],  Jf=Jf[[3]], O=O[[3]], ID=IDdat[[3]], Kref=298.15, R=0.008314, Tref=25) #bro
datalist4<-list(N=length(A[[4]]), NID=length(unique(IDdat[[4]])), An=A[[4]], Ca=CP[[4]], T=T[[4]],K=K[[4]], g=g[[4]],  Jf=Jf[[4]], O=O[[4]], ID=IDdat[[4]], Kref=298.15, R=0.008314, Tref=25)  #cab
datalist5<-list(N=length(A[[5]]), NID=length(unique(IDdat[[5]])), An=A[[5]], Ca=CP[[5]], T=T[[5]],K=K[[5]], g=g[[5]], Jf=Jf[[5]], O=O[[5]], ID=IDdat[[5]], Kref=298.15, R=0.008314, Tref=25)  #oil
datalist6<-list(N=length(A[[6]]), NID=length(unique(IDdat[[6]])), An=A[[6]], Ca=CP[[6]], T=T[[6]],K=K[[6]], g=g[[6]],  Jf=Jf[[6]], O=O[[6]], ID=IDdat[[6]], Kref=298.15, R=0.008314, Tref=25) #tur
...
####### Plot results #####
#plot(mcmc_samples1)
mcmcChain_r301_7 = as.matrix( mcmc_samples1)
chainLength = NROW(mcmcChain_r301_7)
# Convert precision (tau) to SD###
sigma =1  / sqrt( mcmcChain_r301_7[, "tau" ] )
mcmcChain_r301_7 = as.data.frame(cbind( mcmcChain_r301_7, sigma ))
g1<-gelman.diag(mcmc_samples1)
###
mcmc_samples2<- coda.samples(model2,
variable.names=parameters,
n.iter=nPerChain , thin=thinSteps )
mcmcChain_r46_7 = as.matrix( mcmc_samples2)
# Convert precision (tau) to SD###
sigma =1  / sqrt( mcmcChain_r46_7[, "tau" ] )
mcmcChain_r46_7 = as.data.frame(cbind( mcmcChain_r46_7, sigma ))
g2<-gelman.diag(mcmc_samples2)
######
mcmc_samples3 <- coda.samples(model3,
variable.names=parameters,
n.iter=nPerChain , thin=thinSteps )
mcmcChain_bro_7 = as.matrix( mcmc_samples3)
# Convert precision (tau) to SD###
sigma = 1  / sqrt( mcmcChain_bro_7[, "tau" ] )
mcmcChain_bro_7 = as.data.frame(cbind( mcmcChain_bro_7, sigma ))
g3<-gelman.diag(mcmc_samples3)
######
mcmc_samples4 <- coda.samples(model4,
variable.names=parameters,
n.iter=nPerChain , thin=thinSteps )
mcmcChain_cab_7 = as.matrix( mcmc_samples4)
# Convert precision (tau) to SD###
sigma = 1  / sqrt( mcmcChain_cab_7[, "tau" ] )
mcmcChain_cab_7 = as.data.frame(cbind( mcmcChain_cab_7, sigma ))
g4<-gelman.diag(mcmc_samples4)
######
mcmc_samples5 <- coda.samples(model5,
variable.names=parameters,
n.iter=nPerChain , thin=thinSteps )
mcmcChain_oil_7 = as.matrix( mcmc_samples5)
# Convert precision (tau) to SD###
sigma = 1  / sqrt( mcmcChain_oil_7[, "tau" ] )
mcmcChain_oil_7 = as.data.frame(cbind( mcmcChain_oil_7, sigma ))
g5<-gelman.diag(mcmc_samples5)
######
mcmc_samples6 <- coda.samples(model6,
variable.names=parameters,
n.iter=nPerChain , thin=thinSteps )
mcmcChain_tur_7 = as.matrix( mcmc_samples6)
# Convert precision (tau) to SD###
sigma = 1  / sqrt( mcmcChain_tur_7[, "tau" ] )
mcmcChain_tur_7 = as.data.frame(cbind( mcmcChain_tur_7, sigma ))
...
write.table(mcmcChain_r301_7,"mcmcChain_r301_7", sep="\t", col.name=TRUE)
write.table(mcmcChain_r46_7,"mcmcChain_r46_7", sep="\t", col.name=TRUE)
write.table(mcmcChain_bro_7,"mcmcChain_bro_7", sep="\t", col.name=TRUE)
write.table(mcmcChain_cab_7,"mcmcChain_cab_7", sep="\t", col.name=TRUE)
write.table(mcmcChain_oil_7,"mcmcChain_oil_7", sep="\t", col.name=TRUE)
write.table(mcmcChain_tur_7,"mcmcChain_tur_7", sep="\t",col.name=TRUE)
...
common_cols <- colnames(mcmcChain_oil_7)
mcmcChain_r301_7<-subset(mcmcChain_r301_7, select = common_cols)
mcmcChain_r46_7<-subset(mcmcChain_r46_7, select = common_cols)
mcmcChain_bro_7<-subset(mcmcChain_bro_7, select = common_cols)
mcmcChain_cab_7<-subset(mcmcChain_cab_7, select = common_cols)
mcmcChain_tur_7<-subset(mcmcChain_tur_7, select = common_cols)
#############################################


#############################################
EstPars1<-apply(mcmcChain_r301_7, 2, median)
EstPars2<-apply(mcmcChain_r46_7, 2, median)
EstPars3<-apply(mcmcChain_bro_7, 2, median)
EstPars4<-apply(mcmcChain_cab_7, 2, median)
EstPars5<-apply(mcmcChain_oil_7, 2, median)
EstPars6<-apply(mcmcChain_tur_7, 2, median)
...
dic_r301_7<-c(sum(dic1$deviance), sum(dic1$penalty),sum(dic1$deviance, dic1$penalty),
...
dic_r46_7<-c(sum(dic1$deviance), sum(dic1$penalty),sum(dic1$deviance, dic1$penalty),
sum(dic1a$deviance), sum(dic1a$penalty), sum(dic1a$deviance, dic1a$penalty))

dic1 <- dic.samples(model3, DICsteps, "pD")
dic1a <- dic.samples(model3, DICsteps, "popt")
dic_bro_7<-c(sum(dic1$deviance), sum(dic1$penalty),sum(dic1$deviance, dic1$penalty),
sum(dic1a$deviance), sum(dic1a$penalty), sum(dic1a$deviance, dic1a$penalty))

dic1 <- dic.samples(model4, DICsteps, "pD")
dic1a <- dic.samples(model4, DICsteps, "popt")
dic_cab_7<-c(sum(dic1$deviance), sum(dic1$penalty),sum(dic1$deviance, dic1$penalty),
sum(dic1a$deviance), sum(dic1a$penalty), sum(dic1a$deviance, dic1a$penalty))

dic1 <- dic.samples(model5, DICsteps, "pD")
dic1a <- dic.samples(model5, DICsteps, "popt")
dic_oil_7<-c(sum(dic1$deviance), sum(dic1$penalty),sum(dic1$deviance, dic1$penalty),
sum(dic1a$deviance), sum(dic1a$penalty), sum(dic1a$deviance, dic1a$penalty))

dic1 <- dic.samples(model6, DICsteps, "pD")
dic1a <- dic.samples(model6, DICsteps, "popt")
dic_tur_7<-c(sum(dic1$deviance), sum(dic1$penalty),sum(dic1$deviance, dic1$penalty),
sum(dic1a$deviance), sum(dic1a$penalty), sum(dic1a$deviance, dic1a$penalty))

dics<-rbind(dic_r301_7, dic_r46_7, dic_bro_7, dic_cab_7, dic_oil_7,dic_tur_7)
colnames(dics)<-c("pD_dev", "pD_pen", "pD_DIC", "popt_dev", "popt_pen", "popt_DIC")

modelname<-rep("CaCc_Temp_Jf",6)
...
write.table(ParEstFull,"ParEstFull_model_7", sep="\t", row.names=F, col.name=TRUE)
...
pdf("~/Documents/ACiP4/Trace_plots/density_r301_7.pdf",
colormodel='cmyk',width=6.25, height=(180))
ggs_density(ggs(mcmc_samples1))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/density_46_7.pdf",
colormodel='cmyk',width=6.25, height=(180))
ggs_density(ggs(mcmc_samples2))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/density_bro_7.pdf", colormodel='cmyk',width=6.25, height=(180))
ggs_density(ggs(mcmc_samples3))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/density_cab_7.pdf", colormodel='cmyk',width=6.25, height=(180))
ggs_density(ggs(mcmc_samples4))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/density_oil_7.pdf", colormodel='cmyk',width=6.25, height=(180))
ggs_density(ggs(mcmc_samples5))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/density_tur_7.pdf", colormodel='cmyk',width=6.25, height=(180))
...
pdf("~/Documents/ACiP4/Trace_plots/cor_r301_7.pdf",
colormodel='cmyk',width=6.25, height=(80))
ggs_crosscorrelation(ggs(mcmc_samples1))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/cor_46_7.pdf", colormodel='cmyk',,width=10, height=(10))
ggs_crosscorrelation(ggs(mcmc_samples2))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/cor_bro_7.pdf", colormodel='cmyk',,width=10, height=(10))
ggs_crosscorrelation(ggs(mcmc_samples3))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/cor_cab_7.pdf", colormodel='cmyk',,width=10, height=(10))
ggs_crosscorrelation(ggs(mcmc_samples4))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/cor_oil_7.pdf", colormodel='cmyk',,width=10, height=(10))
ggs_crosscorrelation(ggs(mcmc_samples5))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/cor_tur_7.pdf", colormodel='cmyk',width=10, height=(10))
ggs_crosscorrelation(ggs(mcmc_samples6))
dev.off()


pdf("~/Documents/ACiP4/Trace_plots/Rhat_r301_7.pdf",
colormodel='cmyk',width=6.25, height=(80))
ggs_Rhat(ggs(mcmc_samples1))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/Rhat_46_7.pdf",
colormodel='cmyk',width=6.25, height=(80))
ggs_Rhat(ggs(mcmc_samples2))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/Rhat_bro_7.pdf", colormodel='cmyk',width=6.25, height=(80))
ggs_Rhat(ggs(mcmc_samples3))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/Rhat_cab_7.pdf", colormodel='cmyk',width=6.25, height=(80))
ggs_Rhat(ggs(mcmc_samples4))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/Rhat_oil_7.pdf", colormodel='cmyk',width=6.25, height=(80))
ggs_Rhat(ggs(mcmc_samples5))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/Rhat_tur_7.pdf", colormodel='cmyk',width=6.25, height=(80))
