...
for (i in 1:N)
{
    ...
    Jmax[i] <- Jmax25[ID[i]]*exp((T[i]-Tref)*EJmax/(Kref*R*K[i]))
    ...
    a2[i]<-(-4.0/g[i])
    b2[i]<-(4.0*Ca[i]) + (8.0*gammaS[i]) + (Jm[i]/g[i]) - (4.0*Rd[i] /g[i])
    c2[i]<-Jm[i] *(gammaS[i] - Ca[i] ) + (Rd[i] *(4.0*Ca[i] + 8.0*gammaS[i]))
...
### normal prior Dark Respiration from LR data (umol m-2 s-1)
mu.Rd25 ~ dnorm(3.97, 1)tau.Rd25 ~ dgamma(9, 16)  # as variance distribution has mean = 1.4 ; SD = .25
