...
gm[i] <- gm25[ID[i]]*exp((T[i]-Tref)*Egm/(Kref*R*K[i]))
...
a2[i]<-(-4.0/gm[i])
b2[i]<-(4.0*CiP[i]) + (8.0*gammaS[i]) + (Jf[i]/gm[i]) - (4.0*Rd[i] /gm[i])
c2[i]<-Jf[i] *(gammaS[i] - CiP[i] ) + (Rd[i] *(4.0*CiP[i] + 8.0*gammaS[i]))
...
for (k in 1:NID){
    ...
    Rd25[k] ~ dnorm(mu.Rd25, tau.Rd25)T(0,)
