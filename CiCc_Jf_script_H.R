...
source("CiCc_Jf_model_H.R") ### Bayesian Modelw/ Temp Dependency & farQ limitation

#### Set up for rjags #########
parameters = c("Vcmax", "Rd","gm","gammaS" ,"Kc", "Ko",
"mu.Vcmax", "mu.Rd","mu.gm","mu.gammaS",
"tau.Vcmax", "tau.Rd","tau.gm","tau.gammaS" ,
"tau") ### pars to be monitored
...
datalist1<-list(N=length(A[[1]]), NID=length(unique(IDdat[[1]])), An=A[[1]], CiP=CP[[1]],  Jf=Jf[[1]], O=O[[1]], ID=IDdat[[1]], Kref=298.15, R=0.008314, Tref=25) #301
datalist2<-list(N=length(A[[2]]), NID=length(unique(IDdat[[2]])), An=A[[2]], CiP=CP[[2]],  Jf=Jf[[2]], O=O[[2]], ID=IDdat[[2]], Kref=298.15, R=0.008314, Tref=25) #46
datalist3<-list(N=length(A[[3]]), NID=length(unique(IDdat[[3]])), An=A[[3]], CiP=CP[[3]],  Jf=Jf[[3]], O=O[[3]], ID=IDdat[[3]], Kref=298.15, R=0.008314, Tref=25) #bro
datalist4<-list(N=length(A[[4]]), NID=length(unique(IDdat[[4]])), An=A[[4]], CiP=CP[[4]],  Jf=Jf[[4]], O=O[[4]], ID=IDdat[[4]], Kref=298.15, R=0.008314, Tref=25)  #cab
datalist5<-list(N=length(A[[5]]), NID=length(unique(IDdat[[5]])), An=A[[5]], CiP=CP[[5]], Jf=Jf[[5]], O=O[[5]], ID=IDdat[[5]], Kref=298.15, R=0.008314, Tref=25)  #oil
datalist6<-list(N=length(A[[6]]), NID=length(unique(IDdat[[6]])), An=A[[6]], CiP=CP[[6]],  Jf=Jf[[6]], O=O[[6]], ID=IDdat[[6]], Kref=298.15, R=0.008314, Tref=25) #turN<-36
...
####### Plot results #####
#plot(mcmc_samples1)
mcmcChain_r301_1 = as.matrix( mcmc_samples1)
chainLength = NROW(mcmcChain_r301_1)
# Convert precision (tau) to SD###
sigma =1  / sqrt( mcmcChain_r301_1[, "tau" ] )
mcmcChain_r301_1 = as.data.frame(cbind( mcmcChain_r301_1, sigma ))
...
variable.names=parameters,
n.iter=nPerChain , thin=thinSteps )
mcmcChain_r46_1 = as.matrix( mcmc_samples2)
# Convert precision (tau) to SD###
sigma =1  / sqrt( mcmcChain_r46_1[, "tau" ] )
mcmcChain_r46_1 = as.data.frame(cbind( mcmcChain_r46_1, sigma ))
...
variable.names=parameters,
n.iter=nPerChain , thin=thinSteps )
mcmcChain_bro_1 = as.matrix( mcmc_samples3)
# Convert precision (tau) to SD###
sigma = 1  / sqrt( mcmcChain_bro_1[, "tau" ] )
mcmcChain_bro_1 = as.data.frame(cbind( mcmcChain_bro_1, sigma ))
...
variable.names=parameters,
n.iter=nPerChain , thin=thinSteps )
mcmcChain_cab_1 = as.matrix( mcmc_samples4)
# Convert precision (tau) to SD###
sigma = 1  / sqrt( mcmcChain_cab_1[, "tau" ] )
mcmcChain_cab_1 = as.data.frame(cbind( mcmcChain_cab_1, sigma ))
...
variable.names=parameters,
n.iter=nPerChain , thin=thinSteps )
mcmcChain_oil_1 = as.matrix( mcmc_samples5)
# Convert precision (tau) to SD###
sigma = 1  / sqrt( mcmcChain_oil_1[, "tau" ] )
mcmcChain_oil_1 = as.data.frame(cbind( mcmcChain_oil_1, sigma ))
...
variable.names=parameters,
n.iter=nPerChain , thin=thinSteps )
mcmcChain_tur_1 = as.matrix( mcmc_samples6)
# Convert precision (tau) to SD###
sigma = 1  / sqrt( mcmcChain_tur_1[, "tau" ] )
mcmcChain_tur_1 = as.data.frame(cbind( mcmcChain_tur_1, sigma ))
...
write.table(mcmcChain_r301_1,"mcmcChain_r301_1", sep="\t", col.name=TRUE)
write.table(mcmcChain_r46_1,"mcmcChain_r46_1", sep="\t", col.name=TRUE)
write.table(mcmcChain_bro_1,"mcmcChain_bro_1", sep="\t", col.name=TRUE)
write.table(mcmcChain_cab_1,"mcmcChain_cab_1", sep="\t", col.name=TRUE)
write.table(mcmcChain_oil_1,"mcmcChain_oil_1", sep="\t", col.name=TRUE)
write.table(mcmcChain_tur_1,"mcmcChain_tur_1", sep="\t", col.name=TRUE)
#########################################################################
...
common_cols <- colnames(mcmcChain_oil_1)
mcmcChain_r301_1<-subset(mcmcChain_r301_1, select = common_cols)
mcmcChain_r46_1<-subset(mcmcChain_r46_1, select = common_cols)
mcmcChain_bro_1<-subset(mcmcChain_bro_1, select = common_cols)
mcmcChain_cab_1<-subset(mcmcChain_cab_1, select = common_cols)
mcmcChain_tur_1<-subset(mcmcChain_tur_1, select = common_cols)
#############################################
...
EstPars1<-apply(mcmcChain_r301_1, 2, median)
EstPars2<-apply(mcmcChain_r46_1, 2, median)
EstPars3<-apply(mcmcChain_bro_1, 2, median)
EstPars4<-apply(mcmcChain_cab_1, 2, median)
EstPars5<-apply(mcmcChain_oil_1, 2, median)
EstPars6<-apply(mcmcChain_tur_1, 2, median)
...
dic_r301_1<-c(sum(dic1$deviance), sum(dic1$penalty),sum(dic1$deviance, dic1$penalty),
...
dic_r46_1<-c(sum(dic1$deviance), sum(dic1$penalty),sum(dic1$deviance, dic1$penalty),
sum(dic1a$deviance), sum(dic1a$penalty), sum(dic1a$deviance, dic1a$penalty))

dic1 <- dic.samples(model3, DICsteps, "pD")
dic1a <- dic.samples(model3, DICsteps, "popt")
dic_bro_1<-c(sum(dic1$deviance), sum(dic1$penalty),sum(dic1$deviance, dic1$penalty),
sum(dic1a$deviance), sum(dic1a$penalty), sum(dic1a$deviance, dic1a$penalty))

dic1 <- dic.samples(model4, DICsteps, "pD")
dic1a <- dic.samples(model4, DICsteps, "popt")
dic_cab_1<-c(sum(dic1$deviance), sum(dic1$penalty),sum(dic1$deviance, dic1$penalty),
sum(dic1a$deviance), sum(dic1a$penalty), sum(dic1a$deviance, dic1a$penalty))

dic1 <- dic.samples(model5, DICsteps, "pD")
dic1a <- dic.samples(model5, DICsteps, "popt")
dic_oil_1<-c(sum(dic1$deviance), sum(dic1$penalty),sum(dic1$deviance, dic1$penalty),
sum(dic1a$deviance), sum(dic1a$penalty), sum(dic1a$deviance, dic1a$penalty))

dic1 <- dic.samples(model6, DICsteps, "pD")
dic1a <- dic.samples(model6, DICsteps, "popt")
dic_tur_1<-c(sum(dic1$deviance), sum(dic1$penalty),sum(dic1$deviance, dic1$penalty),
sum(dic1a$deviance), sum(dic1a$penalty), sum(dic1a$deviance, dic1a$penalty))


dics<-rbind(dic_r301_1, dic_r46_1, dic_bro_1, dic_cab_1, dic_oil_1,dic_tur_1)
...
write.table(ParEstFull,"ParEstFull_model_1", sep="\t", col.name=TRUE,row.names = F)
...
pdf("~/Documents/ACiP4/Trace_plots/density_r301_1.pdf",
colormodel='cmyk',width=6.25, height=(80))
ggs_density(ggs(mcmc_samples1))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/density_46_1.pdf",
colormodel='cmyk',width=6.25, height=(80))
ggs_density(ggs(mcmc_samples2))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/density_bro_1.pdf", colormodel='cmyk',width=6.25, height=(80))
ggs_density(ggs(mcmc_samples3))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/density_cab_1.pdf", colormodel='cmyk',width=6.25, height=(80))
ggs_density(ggs(mcmc_samples4))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/density_oil_1.pdf", colormodel='cmyk',width=6.25, height=(80))
ggs_density(ggs(mcmc_samples5))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/density_tur_1.pdf", colormodel='cmyk',width=6.25, height=(80))
...
pdf("~/Documents/ACiP4/Trace_plots/cor_r301_1.pdf",
colormodel='cmyk',width=10, height=(10))
ggs_crosscorrelation(ggs(mcmc_samples1))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/cor_46_1.pdf", colormodel='cmyk',,width=10, height=(10))
ggs_crosscorrelation(ggs(mcmc_samples2))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/cor_bro_1.pdf", colormodel='cmyk',,width=10, height=(10))
ggs_crosscorrelation(ggs(mcmc_samples3))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/cor_cab_1.pdf", colormodel='cmyk',,width=10, height=(10))
ggs_crosscorrelation(ggs(mcmc_samples4))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/cor_oil_1.pdf", colormodel='cmyk',,width=10, height=(10))
ggs_crosscorrelation(ggs(mcmc_samples5))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/cor_tur_1.pdf", colormodel='cmyk',width=10, height=(10))
ggs_crosscorrelation(ggs(mcmc_samples6))
dev.off()


pdf("~/Documents/ACiP4/Trace_plots/Rhat_r301_1.pdf",
colormodel='cmyk',width=6.25, height=(20))
ggs_Rhat(ggs(mcmc_samples1))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/Rhat_46_1.pdf",
colormodel='cmyk',width=6.25, height=(20))
ggs_Rhat(ggs(mcmc_samples2))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/Rhat_bro_1.pdf", colormodel='cmyk',width=6.25, height=(20))
ggs_Rhat(ggs(mcmc_samples3))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/Rhat_cab_1.pdf", colormodel='cmyk',width=6.25, height=(20))
ggs_Rhat(ggs(mcmc_samples4))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/Rhat_oil_1.pdf", colormodel='cmyk',width=6.25, height=(20))
ggs_Rhat(ggs(mcmc_samples5))
dev.off()

pdf("~/Documents/ACiP4/Trace_plots/Rhat_tur_1.pdf", colormodel='cmyk',width=6.25, height=(20))
